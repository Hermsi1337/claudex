...
Every Codex job runs in the background and leaves `<name>.out` (full output) and `<name>.exit` (exit code) next to its prompt file; a watcher process exits the moment those sentinels appear. Completion is read from the sentinel files, never from background-task notifications — those go missing often enough (Windows, subagents) to strand a session waiting on a Codex call that already finished.
...
- On native Windows, use Codex ≥ 0.145: the sandbox works natively and `codex exec` adds trust entries itself, so no setup is needed. Older Codex versions degrade `workspace-write` to read-only there regardless of trust entries — `/claudex` then falls back per call to `--dangerously-bypass-approvals-and-sandbox` with a notice recommending a Codex update, and `/claudex:setup` flags the outdated version.
