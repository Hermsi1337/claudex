...
This is the **claudex** Claude Code plugin. It turns Claude into an orchestrator that delegates implementation work and broad codebase research to OpenAI Codex (via the local `codex` CLI), handles documentation/conceptual work itself, and reviews Codex' diff at the end.
...
- Research subtasks: broad codebase exploration (mapping a subsystem, finding all implementations of a pattern, tracing data flow) is delegated to `codex exec --sandbox read-only` — **never** to Claude subagents. The Agent tool is banned outright in claudex mode, sticky follow-ups included; quick lookups (a targeted grep or two, ≤2 known files) stay inline in Claude. Read-only needs no trust-list entry, and the sandbox-bypass fallback must never fire on a research call (a read-only job hitting a write rejection is a prompt bug, not a trust problem). Research has **two forms** with different prompt shapes — the split exists to prevent anchoring (Claude pre-chewing findings until Codex only rubber-stamps them). **Enumeration** (closed list questions) gets seed `path:line` hits from one orchestrator-side grep, framed as an incomplete list to verify and extend; always `standard` effort. **Investigation** (open how/why questions) gets **no seeds and no hypothesis** — the question is stated neutrally and Codex forms its own picture; may be tagged `complex` (→ the normal `high` escalation, config-peek guard included) when it needs cross-module causal reasoning. Hypothesis confirmation is not research at all: Claude verifies suspicions inline or reframes them as neutral questions — enforced by a pre-prompt self-check (name the form in one word; if the answer is predictable in one sentence, it's a confirmation prompt in the making). Mixed questions are split into one subtask per form, chained via the already-known block when one part feeds the other. Four blocks are shared by both forms: mission context (neutral question + what the answer feeds), repo orientation (structure snapshot + negative scope; `--cd` only when the *task* confines scope, never because Claude guesses where the answer lives), an output contract (`path:line — one-line answer`, "Open questions", a mandatory "Contradicts expectations" section, hard length cap that limits prose not findings, "your text output IS the deliverable"), and an already-known chaining block labelled as unverified prior findings to build on and challenge. Findings are leads, not ground truth — Claude spot-checks a cited `path:line` (for investigations: the load-bearing causal claim) before embedding them into an implementation prompt. Same token economy as the hand-off discipline: orientation costs Claude a few hundred tokens so Codex doesn't explore cold, and none of the exploration lands on Claude's bill.
- Multi-repo tasks: `--cd` accepts any absolute path, so a task spanning several repos becomes one Codex call per repo — parallel by default (different repos are file-disjoint by definition), each prompt carrying a files-in-scope list relative to its own repo root. The trivial-code exception is disabled the moment more than one repo is involved, and Claude never edits foreign repos inline — "the repo is outside my working directory" is never a reason to keep work in Claude. Foreign-repo context is gathered via read-only research calls with `--cd <repo>` (the research call also reports that repo's conventions; Codex auto-reads its `AGENTS.md` on the implementation call), review runs per repo via `git -C <repo> diff`, and the Phase 5 report groups entries per repo. Trust is per repo on macOS/Linux: `/claudex:setup [dir]` can check/trust a directory other than the cwd; otherwise the bypass fallback fires per repo and the notice must name which repo it fired for. On native Windows the platform-split sandbox bullet above wins — the preemptive bypass covers every write-capable call regardless of repo, and no per-repo `/claudex:setup` recommendation is made.
