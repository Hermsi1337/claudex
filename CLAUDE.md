...
- Codex' `--sandbox workspace-write` only actually permits writes when the project is in `~/.codex/config.toml` under `[projects.'<path>'] trust_level = "trusted"` (or the platform-equivalent form — Windows native uses lowercased backslash paths in TOML literal strings; Mac/Linux uses the absolute path in basic strings). Without trust, every patch is rejected with `patch rejected: writing is blocked by read-only sandbox`. The runtime `-c projects.X.trust_level="trusted"` override has been observed not to work reliably on Windows, so don't rely on it. Two integrated mitigations: (1) `/claudex:setup` Step 4 detects untrusted projects and offers to append the entry (with explicit user consent via `AskUserQuestion`); (2) Phase 3 has a runtime fallback that re-runs the same `codex exec` call once with `--dangerously-bypass-approvals-and-sandbox` if and only if the output contains the exact substring `patch rejected: writing is blocked by read-only sandbox`. The fallback must be reported prominently in Phase 5 so the user knows the call ran with sandboxing disabled. The bypass flag is documented on `codex exec --help` for Codex CLI ≥ 0.128 — keep the existing rule that the legacy `--full-auto` preset is gone, but no longer claim sandbox+approval flags alone are sufficient.
...
- Research subtasks: broad codebase exploration (mapping a subsystem, finding all implementations of a pattern, tracing data flow) is delegated to `codex exec --sandbox read-only` — **never** to Claude subagents. The Agent tool is banned outright in claudex mode, sticky follow-ups included; quick lookups (a targeted grep or two, ≤2 known files) stay inline in Claude. Read-only needs no trust-list entry, and the sandbox-bypass fallback must never fire on a research call (a read-only job hitting a write rejection is a prompt bug, not a trust problem). Research subtasks are always `standard` effort — only an explicit `--effort` changes that. The research prompt template has five mandatory blocks: mission context (what the answer feeds), repo orientation (structure snapshot + negative scope + `--cd` where confined), seed findings from one orchestrator-side grep (`path:line` only, no contents), an output contract (`path:line — one-line answer`, "Open questions" section, hard length cap, "your text output IS the deliverable"), and an already-known chaining block for follow-up calls. Findings are leads, not ground truth — Claude spot-checks a cited `path:line` before embedding them into an implementation prompt. Same token economy as the hand-off discipline: Claude spends a few hundred tokens on orientation + seeds so Codex doesn't burn thousands exploring cold, and none of the exploration lands on Claude's bill.
- Multi-repo tasks: `--cd` accepts any absolute path, so a task spanning several repos becomes one Codex call per repo — parallel by default (different repos are file-disjoint by definition), each prompt carrying a files-in-scope list relative to its own repo root. The trivial-code exception is disabled the moment more than one repo is involved, and Claude never edits foreign repos inline — "the repo is outside my working directory" is never a reason to keep work in Claude. Foreign-repo context is gathered via read-only research calls with `--cd <repo>` (the research call also reports that repo's conventions; Codex auto-reads its `AGENTS.md` on the implementation call), review runs per repo via `git -C <repo> diff`, and the Phase 5 report groups entries per repo. Trust is per repo: `/claudex:setup [dir]` can check/trust a directory other than the cwd; otherwise the bypass fallback fires per repo and the notice must name which repo it fired for.
