...
- [OpenAI Codex CLI](https://github.com/openai/codex) **≥ 0.128** installed and authenticated locally (**≥ 0.145 recommended on native Windows** — that's where Codex' Windows sandbox starts working; older versions push every write onto the bypass fallback)
  - `npm install -g @openai/codex`
  - `codex login`
  - The plugin uses `codex exec --sandbox workspace-write [--cd <dir>] [--model <name>] - < <prompt-dir>/<id>.md`. Prompts are always fed via stdin from a file (sidesteps shell-escaping bugs on prompts that contain backticks, `$`, or quotes). Each `/claudex` call writes into its own per-call subdirectory `/tmp/claudex-prompts/<call_id>/` (or the Windows-resolved equivalent under `%TEMP%\claudex-prompts\<call_id>\` on Git Bash) so concurrent Claude Code sessions don't pile prompt files on top of each other. `call_id` is timestamped (`20260509-171152-89c895`), so `ls -t /tmp/claudex-prompts/` shows the most recent call on top. Each Codex job additionally leaves `<name>.out` (its full output) and `<name>.exit` (its exit code) next to the prompt file — claudex detects job completion through these sentinel files instead of trusting background-task notifications, which have been observed to go missing (especially on Windows and inside subagents), leaving sessions waiting forever on a Codex call that already finished. Older Codex versions had a `--full-auto` preset that this plugin no longer relies on.
  ...
  - **Native Windows:** works out of the box with Codex ≥ 0.145 — the sandbox runs natively and `codex exec` adds the trust entry itself on first run. On older Codex versions `workspace-write` degrades to read-only regardless of trust entries; `/claudex` then falls back per call to `--dangerously-bypass-approvals-and-sandbox` with a notice recommending a Codex update. See "Sandbox & trust" below.
...
On macOS/Linux, each target repo needs its own entry in your Codex trust list: run `/claudex:setup <repo-path>` once per repo, or accept the per-repo sandbox-bypass fallback (with a notice naming the repo) until you do. On native Windows with Codex ≥ 0.145 no per-repo setup is needed — `codex exec` manages trust entries itself (see "Sandbox & trust" below).
...
**Native Windows.** Fixed as of Codex CLI 0.145 (verified on 0.145.0): `--sandbox workspace-write` works natively — the session header reports `sandbox: workspace-write` — and `codex exec` appends the project's trust entry to `~/.codex/config.toml` itself on first run, so neither a manual trust step nor any bypass flag is needed. On older Codex versions (degradation confirmed through 0.128), `workspace-write` fell back to read-only unconditionally, trust entries included; there `/claudex` hits the same runtime fallback as an untrusted macOS/Linux project on every write-capable call, and the per-call notice recommends updating Codex (`npm install -g @openai/codex`) — on Windows the durable fix is the update, not trust entries. `/claudex:setup` checks your installed version and tells you which side of the fix you're on.

**Claude Code auto mode.** If your Claude Code session runs in auto mode, its permission classifier blocks nested agents launched with sandboxing and approvals disabled — which is exactly what the (rare) bypass-fallback retry looks like. `/claudex` won't try to sneak past the block and won't silently do the work inline; it surfaces the denial together with the remedies: retry the denied call via `/permissions` → "Recently denied" → `r`, add an `autoMode.allow` prose entry to `~/.claude/settings.json` (the classifier ignores project-level settings), or run that task outside auto mode. Regular sandboxed Codex calls don't trip the classifier.
...
Completion is detected via **sentinel files**: every Codex job writes its output to `<name>.out` and its exit code to `<name>.exit` in the per-call prompt directory, and a small watcher process exits the moment the sentinels appear. The orchestrator never sleeps on background-task notifications — those have been observed to go missing (notably on Windows and inside subagents), which used to leave sessions waiting forever on a Codex call that had long finished.
