...
- [OpenAI Codex CLI](https://github.com/openai/codex) **≥ 0.128** installed and authenticated locally
  ...
  - `codex login`
  - The plugin uses `codex exec --sandbox workspace-write [--cd <dir>] [--model <name>] - < <prompt-dir>/<id>.md`. Prompts are always fed via stdin from a file (sidesteps shell-escaping bugs on prompts that contain backticks, `$`, or quotes). The prompt directory is `/tmp/claudex-prompts` on macOS/Linux and the resolved Windows equivalent on Git Bash (typically under `%TEMP%\claudex-prompts`); `/claudex` resolves it once per call so the Write tool and the Bash redirect agree on file location. Older Codex versions had a `--full-auto` preset that this plugin no longer relies on.
